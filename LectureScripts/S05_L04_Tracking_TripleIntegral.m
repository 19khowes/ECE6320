function S05_L04_Tracking_TripleIntegral()
close all;
...
% Augmented dynamics
Abar = [A zeros(2,3); C zeros(1,3); 0 0 1 0 0; 0 0 0 1 0];
Bbar = [B; 0; 0; 0];
Ebar = [E; 0; 0; 0];
...
plot(t_mat, x_mat(1,:), 'linewidth', 2);
ylabel('y, r, and w');

subplot(2,1,2); % Plot error
plot(t_mat, err, 'linewidth', 2);
ylabel('Error (y - r)');
xlabel('Time(s)');
...
%% Dynamics function
    function xbardot = stateDynamics(t, xbar, K, rSol, wSol)
        ...
        xi = xbar - [r; 0; 0; 0]; % Get augmented state
        ...
        sig3_dot = xbar(4);
        xdot = A*x + B*u + E*w(1);
        xbardot = [xdot; sig1_dot; sig2_dot; sig3_dot];
    end
